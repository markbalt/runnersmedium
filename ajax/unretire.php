...
if (isset($_GET['id']) && is_numeric($_GET['id'])) :
	...
	if ($conn->getRow($result) == 0) {
		...
		// retire shoe
		$conn->query('UPDATE shoes SET retired = 0 WHERE id = '.mysql_real_escape_string($id).' AND user = '.mysql_real_escape_string($user->ID()));
		echo json_encode(array('status' => 'ok', 'id' => format($id), 'text' => 'the shoe is active now'));
	}
